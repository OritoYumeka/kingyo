...
var $author$project$MainOct09$init = function (flags) {
...
var $author$project$Types$KingyoCaught = function (a) {
	return {$: 'KingyoCaught', a: a};
};
var $author$project$Types$KingyoMoved = function (a) {
	return {$: 'KingyoMoved', a: a};
};
...
var $author$project$MainOct09$kingyoCaught = _Platform_incomingPort(
	'kingyoCaught',
	A2(
					...
					return A2(
						...
						function (id) {
							return $elm$json$Json$Decode$succeed(
								{id: id, kingyos: kingyos, points: points});
						},
...
var $author$project$MainOct09$kingyoInfo = _Platform_incomingPort(
	'kingyoInfo',
	$elm$json$Json$Decode$list(
		A2(
			$elm$json$Json$Decode$andThen,
...
var $author$project$MainOct09$moveInfo = _Platform_incomingPort(
	'moveInfo',
	A2(
		$elm$json$Json$Decode$andThen,
		function (y) {
...
var $author$project$MainOct09$skywayId = _Platform_incomingPort(
	'skywayId',
	A2(
		$elm$json$Json$Decode$andThen,
		function (num) {
...
var $author$project$MainOct09$subscriptions = function (_v0) {
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				$author$project$MainOct09$moveInfo($author$project$Types$Recv),
				$author$project$MainOct09$kingyoInfo($author$project$Types$KingyoMoved),
				$author$project$MainOct09$kingyoCaught($author$project$Types$KingyoCaught),
				$author$project$MainOct09$skywayId($author$project$Types$IdDefined),
				A2($elm$time$Time$every, 100, $author$project$Types$Tick)
			]));
};
var $author$project$Types$KingyoGenerated = function (a) {
	return {$: 'KingyoGenerated', a: a};
};
...
var $author$project$MainOct09$caught = _Platform_outgoingPort(
	'caught',
	function ($) {
...
var $author$project$MainOct09$join = _Platform_outgoingPort('join', $elm$json$Json$Encode$string);
var $author$project$Types$Vec2D = F2(
	function (x, y) {
		return {x: x, y: y};
	});
...
var $author$project$MainOct09$pondHeight = 800;
var $author$project$MainOct09$pondWidth = 800;
var $author$project$MainOct09$kingyoStep = function (kingyo) {
	var newVy = ((_Utils_cmp(kingyo.pos.y + kingyo.v.y, $author$project$MainOct09$pondHeight) > 0) || ((kingyo.pos.y + kingyo.v.y) < 0)) ? (-kingyo.v.y) : kingyo.v.y;
	var newVx = ((_Utils_cmp(kingyo.pos.x + kingyo.v.x, $author$project$MainOct09$pondWidth) > 0) || ((kingyo.pos.x + kingyo.v.x) < 0)) ? (-kingyo.v.x) : kingyo.v.x;
	var newPos = A2($author$project$Types$Vec2D, kingyo.pos.x + kingyo.v.x, kingyo.pos.y + kingyo.v.y);
	return _Utils_update(
		kingyo,
		{
			pos: newPos,
			v: A2($author$project$Types$Vec2D, newVx, newVy)
		});
};
...
var $author$project$MainOct09$randomKingyo = A6(
	$elm$random$Random$map5,
	F5(
		function (x, y, vx, vy, level) {
			return A3(
				$author$project$Types$Kingyo,
				A2($author$project$Types$Vec2D, x, y),
				A2($author$project$Types$Vec2D, vx, vy),
				level);
		}),
...
var $author$project$MainOct09$sendKingyo = _Platform_outgoingPort(
	'sendKingyo',
	$elm$json$Json$Encode$list(
		function ($) {
			return $elm$json$Json$Encode$object(
...
var $author$project$MainOct09$sendXY = _Platform_outgoingPort(
	'sendXY',
	function ($) {
		return $elm$json$Json$Encode$object(
			_List_fromArray(
...
var $author$project$MainOct09$sukuu = F2(
...
var $author$project$MainOct09$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'RoomChanged':
				var room = msg.a;
			...
			case 'NameChanged':
				...
				return _Utils_Tuple2(
					_Utils_update(
						...
						{name: name}),
					$elm$core$Platform$Cmd$none);
			case 'KingyoGenerated':
				var newKingyos = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
					...
					$author$project$MainOct09$sendXY(
						{
			...
			case 'Tick':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							kingyos: A2($elm$core$List$map, $author$project$MainOct09$kingyoStep, model.kingyos)
						}),
					model.host ? $author$project$MainOct09$sendKingyo(model.kingyos) : $elm$core$Platform$Cmd$none);
			case 'Join':
				return _Utils_Tuple2(
					model,
					$author$project$MainOct09$join(model.room));
			case 'IdDefined':
				var info = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							host: info.num === 1,
							id: $elm$core$Maybe$Just(info.id)
						}),
					(info.num === 1) ? A2(
						$elm$random$Random$generate,
			...
			case 'Recv':
				...
				var exist = A2(
					$elm$core$Debug$log,
					'move',
					0 < $elm$core$List$length(
						A2(
							$elm$core$List$filter,
							function (p) {
								return _Utils_eq(p.name, info.name);
							},
							model.players)));
				return _Utils_Tuple2(
					exist ? _Utils_update(
			...
			case 'KingyoCaught':
				...
				var updatedPlayers = A2(
					$elm$core$List$map,
					function (p) {
						return _Utils_eq(p.id, data.id) ? _Utils_update(
							p,
							{points: data.points}) : p;
					},
					model.players);
				var dummy = A2($elm$core$Debug$log, '', data);
				return model.host ? _Utils_Tuple2(
					_Utils_update(
						model,
						{kingyos: data.kingyos, players: updatedPlayers}),
					$author$project$MainOct09$sendKingyo(data.kingyos)) : _Utils_Tuple2(
					_Utils_update(
						model,
			...
			case 'Up':
				...
				var y = _v2.b;
				var newKingyos = A2(
					$author$project$MainOct09$sukuu,
					_Utils_Tuple2(x, y),
					model.kingyos);
						...
						F2(
							function (i, k) {
								return _Utils_update(
									k,
									{
										pos: {x: (i + 1) * 100, y: $author$project$MainOct09$pondHeight + 100}
									});
							}),
						model.tsukamaeta),
					model.kingyos);
				...
				return _Utils_Tuple2(
					...
					$author$project$MainOct09$caught(
						{
							id: function () {
								var _v3 = model.id;
								if (_v3.$ === 'Nothing') {
...
var $author$project$MainOct09$coinsView = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
...
var $author$project$MainOct09$pointsView = function (player) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
...
var $author$project$MainOct09$relativePos = function (event) {
	return event.pointer.offsetPos;
};
var $elm$svg$Svg$Attributes$strokeDasharray = _VirtualDom_attribute('stroke-dasharray');
var $author$project$MainOct09$myAmiView = function (model) {
	return A2(
		$elm$svg$Svg$circle,
		_List_fromArray(
			[
				...
				A2($elm$core$Basics$composeR, $author$project$MainOct09$relativePos, $author$project$Types$Down)),
				$mpizenberg$elm_pointer_events$Html$Events$Extra$Pointer$onUp(
				A2($elm$core$Basics$composeR, $author$project$MainOct09$relativePos, $author$project$Types$Up)),
				$mpizenberg$elm_pointer_events$Html$Events$Extra$Pointer$onMove(
				A2($elm$core$Basics$composeR, $author$project$MainOct09$relativePos, $author$project$Types$Move))
			]),
		_List_Nil);
};
var $author$project$MainOct09$namiView = _List_fromArray(
	[
		A2(
		$elm$svg$Svg$circle,
		_List_fromArray(
...
var $author$project$MainOct09$propagate = F3(
	function (x, y, moving) {
		return A2(
...
var $author$project$MainOct09$toOke = F2(
	function (_v0, hiki) {
		var x = _v0.a;
		var y = _v0.b;
		return A3(
			...
			_List_fromArray(
				[
					...
					$andrewMacmurray$elm_simple_animation$Simple$Animation$Property$y($author$project$MainOct09$pondHeight + 100)
				]));
	});
var $author$project$MainOct09$tsukamaetaKingyoView = F2(
	function (hiki, kingyo) {
		...
		var px = kingyo.pos.x;
		return A3(
			$author$project$MainOct09$animatedG,
			A2(
				$author$project$MainOct09$toOke,
				_Utils_Tuple2(px, py),
				hiki),
			_List_fromArray(
				[
...
var $author$project$MainOct09$pondView = function (model) {
	return A2(
		$elm$svg$Svg$svg,
		_List_fromArray(
			[
				$elm$svg$Svg$Attributes$width(
				$elm$core$String$fromInt($author$project$MainOct09$pondWidth)),
				$elm$svg$Svg$Attributes$height(
				$elm$core$String$fromInt($author$project$MainOct09$pondHeight)),
		...
		_Utils_ap(
			_List_fromArray(
				[
					...
					A3(
					$author$project$MainOct09$animatedG,
					A3($author$project$MainOct09$propagate, model.x, model.y, model.moving),
					_List_Nil,
					$author$project$MainOct09$namiView),
					$author$project$MainOct09$myAmiView(model)
				]),
			_Utils_ap(
				A2($elm$core$List$map, $author$project$MainOct09$amiView, model.players),
				_Utils_ap(
					_List_fromArray(
						[
							A2(
...
var $author$project$MainOct09$ifIsEnter = function (msg) {
	return A2(
		$elm$json$Json$Decode$andThen,
		function (key) {
...
var $author$project$MainOct09$roomNameInputView = function (model) {
	return _List_fromArray(
		[
			A2(
			$elm$html$Html$input,
			_List_fromArray(
				[
					...
					$elm$html$Html$Events$on,
					'keydown',
					$author$project$MainOct09$ifIsEnter($author$project$Types$Join)),
...
var $author$project$MainOct09$view = function (model) {
	return A2(
		...
		function () {
			var _v0 = model.id;
			if (_v0.$ === 'Nothing') {
				return $author$project$MainOct09$roomNameInputView(model);
			} else {
				var id = _v0.a;
				return _List_fromArray(
					[
						...
						$elm$html$Html$div,
						_List_Nil,
						A2($elm$core$List$map, $author$project$MainOct09$pointView, model.players)),
						A2(
						$elm$html$Html$div,
						_List_fromArray(
							[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$author$project$MainOct09$pondView(model)
									])),
								A2(
								$elm$html$Html$div,
								_List_fromArray(
...
var $author$project$MainOct09$main = $elm$browser$Browser$element(
	{init: $author$project$MainOct09$init, subscriptions: $author$project$MainOct09$subscriptions, update: $author$project$MainOct09$update, view: $author$project$MainOct09$view});
_Platform_export({'MainOct09':{'init':$author$project$MainOct09$main(
